...
@spatial class Sobel extends SpatialApp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val r = 1500 // args(0).to[Int]
    val c = 1500 // args(1).to[Int]
    ...
    Accel {
      ...
      Foreach(R by row_stride par row_par){ rr =>
        ...
        val kh = LUT[T](3,3)(kh_data.flatten.map(_.to[T]):_*)
        val kv = LUT[T](3,3)(kv_data.flatten.map(_.to[T]):_*)
        ...
        Foreach(-2 until rows_todo) { r =>
          ...
          Foreach(0 until C) { c =>
            ...
            val horz = Reduce(Reg[T])(Kh by 1 par par_Kh, Kw by 1 par par_Kw){(i,j) =>
              sr(i,j) * kh(i,j)
            }{_+_}
            val vert = Reduce(Reg[T])(Kh by 1 par par_Kh, Kw by 1 par par_Kw){(i,j) =>
              sr(i,j) * kv(i,j)
            }{_+_}

            // Store abs sum into answer memory
            lineOut(c) = mux(r + rr < 2 || r + rr >= R-2, 0.to[T], abs(horz.value) + abs(vert.value))
          }
